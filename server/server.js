...
// ---------------- HTML (viewable / interactive) ----------------
// The PDF endpoint renders every row onto a SINGLE page and auto-compresses
// the row height + font (down to ~5pt with ellipsis) to force it to fit,
// which is why long Section/Status values truncate and columns become
// unreadable on dense registers.  HTML is the fix: a self-contained,
// fully-scrollable page with the full text, sortable columns, a live
// search box, red-outlined Section + Status cells (matching the printed
// register), and a Print / Save-as-PDF button.  No data is ever clipped.
const HTML_RED_COLUMNS = new Set(['usSection', 'status']); // mirror the PDF's red boxes
...
function buildHtmlReport({ rows, columns, station, officer, generatedAt, filters }) {
  const today = new Date().toISOString().slice(0, 10);
  const filename = reportFileName('case-property', 'html', filters);

  const headCells = columns.map(c =>
    `<th data-key="${c.key}"${HTML_RED_COLUMNS.has(c.key) ? ' class="red-col"' : ''}>${escapeHtml(c.label)}<span class="sortcaret"></span></th>`
  ).join('');

  // Pre-compute red-column flag per column index so each body cell matches.
  const redIdx = new Set(columns.map((c, i) => HTML_RED_COLUMNS.has(c.key) ? i : -1).filter(i => i >= 0));

  const bodyRows = rows.map((r, ri) => {
    const cells = columns.map((c, ci) =>
      `<td class="${redIdx.has(ci) ? 'red-col' : ''}">${escapeHtml(r[c.key])}</td>`
    ).join('');
    return `<tr class="${ri % 2 ? 'odd' : 'even'}">${cells}</tr>`;
  }).join('');

  const metaBits = [`Generated by ${escapeHtml(officer)} · ${generatedAt} · ${rows.length} item(s)`];
  if (filters.section && filters.section !== 'ALL') metaBits.push(`Part ${escapeHtml(String(filters.section))}`);
  if (filters.status && filters.status !== 'all') metaBits.push(escapeHtml(String(filters.status)));
...
<title>${escapeHtml(station)} — Case Property Register</title>
<style>
  :root{
    --navy:#14243D; --cream:#F0EBDD; --line:#C9BFA0; --ink:#2B2B28;
    --grey:#5C5A4E; --red:#C0392B; --even:#FFFFFF; --odd:#F6F1E6;
  }
  *{ box-sizing:border-box; }
  body{ margin:0; font-family:"Segoe UI",Roboto,Helvetica,Arial,sans-serif; color:var(--ink); background:#f4f2ec; }
  header{
    position:sticky; top:0; z-index:5; background:#fff;
    border-bottom:1px solid var(--line);
    padding:14px 24px 10px; box-shadow:0 1px 4px rgba(0,0,0,.06);
  }
  .station{ font-size:22px; font-weight:700; color:var(--navy); text-align:center; letter-spacing:.3px; }
  .doctitle{ font-size:13px; font-weight:600; color:var(--navy); text-align:center; }
  .meta{ font-size:11px; color:var(--grey); text-align:center; margin-top:2px; }
  .toolbar{ display:flex; gap:10px; align-items:center; margin-top:10px; flex-wrap:wrap; }
  .toolbar input[type=search]{
    flex:1 1 260px; min-width:200px; padding:7px 10px; border:1px solid var(--line);
    border-radius:6px; font-size:13px;
  }
  .toolbar .spacer{ flex:1; }
  .btn{
    cursor:pointer; border:1px solid transparent; border-radius:6px; padding:7px 12px;
    font-size:13px; font-weight:600; color:#fff;
  }
  .btn-print{ background:var(--navy); }
  .btn-html{ background:#2f6fb0; }
  .btn-excel{ background:#1f7a3d; }
  .btn-pdf{ background:var(--red); }
  .btn:active{ transform:translateY(1px); }
  .count{ font-size:12px; color:var(--grey); margin-left:6px; }
  .tablewrap{ padding:14px 24px 40px; overflow-x:auto; }
  table{ border-collapse:collapse; width:100%; min-width:980px; font-size:12.5px; background:#fff;
    box-shadow:0 1px 3px rgba(0,0,0,.08); }
  thead th{
    position:sticky; top:118px; z-index:3; background:var(--navy); color:#FAF7EE;
    text-align:left; font-weight:700; padding:9px 10px; white-space:nowrap; cursor:pointer;
    border-right:1px solid #2c3e5e; user-select:none;
  }
  thead th .sortcaret{ font-size:10px; opacity:.7; margin-left:4px; }
  tbody td{ padding:8px 10px; border:1px solid var(--line); vertical-align:top; line-height:1.35; }
  tbody tr.even td{ background:var(--even); }
  tbody tr.odd  td{ background:var(--odd); }
  tbody tr:hover td{ background:#fff6df; }
  /* Red-outlined columns (Section + Status) — mirrors the printed register. */
  th.red-col, td.red-col{ box-shadow:inset 0 0 0 1.5px var(--red); }
  td.red-col{ font-weight:600; color:#7a241b; }
  .empty{ text-align:center; color:var(--grey); padding:30px; font-style:italic; }
  /* Print: clean, one row per page-ish, no toolbar. */
  @media print{
    body{ background:#fff; }
...
<body>
<header>
  <div class="station">${escapeHtml(station)}</div>
  <div class="doctitle">Case Property Register</div>
  <div class="meta">${metaLine}</div>
  <div class="toolbar">
    <input type="search" id="q" placeholder="Search any column…" aria-label="Search the register" />
    <span class="count" id="count"></span>
    <span class="spacer"></span>
    <button class="btn btn-print" onclick="window.print()" title="Print or Save as PDF">🖨 Print / Save PDF</button>
    <a class="btn btn-excel" href="?format=xlsx">⬇ Excel</a>
    <a class="btn btn-pdf" href="?format=pdf">⬇ PDF</a>
  </div>
</header>
<main class="tablewrap">
  <table id="register">
    <thead><tr>${headCells}</tr></thead>
    <tbody>${bodyRows || `<tr><td class="empty" colspan="${columns.length}">No cases match the applied filters.</td></tr>`}</tbody>
  </table>
</main>
<script>
  (function(){
    ...
    function render(){
      ...
      var vis=allRows.filter(function(r){
        if(!term) return true;
        return Array.prototype.some.call(r.cells,function(c){ return c.textContent.toLowerCase().indexOf(term)>-1; });
      });
      allRows.forEach(function(r){ r.style.display='none'; });
      vis.forEach(function(r){ r.style.display=''; });
      count.textContent=vis.length+' of '+allRows.length+' shown';
    }
    q.addEventListener('input', render);
    Array.prototype.forEach.call(headers,function(th,i){
      th.addEventListener('click',function(){
        sortDir[i]=(sortDir[i]==='asc')?'desc':'asc';
        var dir=sortDir[i];
        var dirn=(dir==='asc')?1:-1;
        vis=allRows.slice().sort(function(a,b){
          var x=a.cells[i].textContent.trim().toLowerCase();
          var y=b.cells[i].textContent.trim().toLowerCase();
          return x<y?-1*dirn:x>y?1*dirn:0;
        });
        vis.forEach(function(r){ tbody.appendChild(r); });
        Array.prototype.forEach.call(headers,function(h){ h.querySelector('.sortcaret').textContent=''; });
        th.querySelector('.sortcaret').textContent=(dir==='asc'?'▲':'▼');
      });
    });
    render();
  })();
...
app.get('/api/reports/case-property', async (req, res, next) => {
  try {
    ...
    if (format === 'html') {
      // Viewable, interactive, self-contained register page.  Full text
      // (nothing clipped like the squashed PDF), sortable columns, live
      // search, red-outlined Section + Status cells, and a Print/Save-PDF
      // button.  See buildHtmlReport() above.
      const html = buildHtmlReport({ rows, columns: REPORT_COLUMNS, station, officer, generatedAt, filters });
      const filename = reportFileName('case-property', 'html', filters);
      res.setHeader('Content-Type', 'text/html; charset=utf-8');
      res.setHeader('Content-Disposition', `inline; filename="${filename}"`);
      res.status(200).send(html);
      auditMm(req, 'report.export', 'case-property', `Exported html: ${rows.length} row(s), filters=${JSON.stringify(filters)}`)
        .catch(e => console.error('[html] audit failed (non-fatal):', e && e.message));
      return;
    }
...
// =================== API: Restore from Archive (Drive) ===================
// Backups live in a Google Drive folder (rclone remote).  These endpoints
// LIST / DOWNLOAD / DELETE stored archives, and RESTORE one into a *staging*
// schema (never the live `public` schema) so a bad archive can't wipe
// production.  Restore is only possible on the operator laptop where
// `psql` + `pg_restore` + rclone are installed (same as pg_dump). On
// Vercel the list/download still work; restore/delete are refused.

function locateRclone() {
  const candidates = [
    join(__dirname, 'bin', 'rclone'),
    join(__dirname, 'bin', 'rclone.exe'),
  ...
  for (const p of candidates) { try { if (existsSync(p)) return p; } catch { /* bad path */ } }
  return null;
}
...
// Strip directory traversal / non-archive characters from a user-supplied
// file name.  Backups are always `backup-<ts>.sql.gz`.
function safeBackupName(name) {
  const m = String(name || '').match(/^backup-\d{4}-\d{2}-\d{2}-\d{4}\.sql\.gz$/);
  return m ? m[0] : null;
}

const RCLONE_BIN = locateRclone();
const PSQL_BIN   = locatePsql();
const RESTORE_SCHEMA = 'public_restore';

// GET /api/backups/list — list archives in the Drive folder (newest first).
app.get('/api/backups/list', async (_req, res, next) => {
  try {
    if (!RCLONE_BIN) {
      return res.json({ ok: true, files: [], note: 'rclone not available on this server.' });
    }
    const out = spawnSync(RCLONE_BIN, [
      ...rcloneConfigArg(), 'lsjson', `${BACKUP_REMOTE}/`,
      '--include', 'backup-*.sql.gz', '--files-only',
    ], { encoding: 'utf8', maxBuffer: 8 * 1024 * 1024 });
    if (out.status !== 0) {
      return res.json({ ok: false, error: 'Could not list Drive backups: ' + (out.stderr || '').toString().slice(0, 300) });
    }
    let files = [];
    try { files = JSON.parse(out.stdout || '[]'); } catch { files = []; }
    files = files
      .filter(f => /^backup-\d{4}-\d{2}-\d{2}-\d{4}\.sql\.gz$/.test(f.Name || ''))
      .map(f => ({
        name: f.Name,
        sizeBytes: f.Size || 0,
        // rclone ModTime is RFC3339 UTC — render in IST for the admin.
        modTime: f.ModTime ? new Date(f.ModTime).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' }) : '—',
      }))
      .sort((a, b) => b.name.localeCompare(a.name));
    res.json({ ok: true, files, remote: BACKUP_REMOTE, folderUrl: BACKUP_FOLDER_URL, restoreSupported: !!PSQL_BIN && !IS_VERCEL });
  } catch (e) { next(e); }
});

// GET /api/backups/:name/download — stream the archive (.sql.gz) to the admin.
app.get('/api/backups/:name/download', async (req, res, next) => {
  try {
    const name = safeBackupName(req.params.name);
    if (!name) return res.status(400).json({ error: 'invalid backup name' });
    if (!RCLONE_BIN) return res.status(503).json({ error: 'rclone not available on this server.' });
    // Verify the file exists first (clean 404 instead of a broken stream).
    const ls = spawnSync(RCLONE_BIN, [...rcloneConfigArg(), 'lsjson', `${BACKUP_REMOTE}/${name}`], { encoding: 'utf8' });
    if (ls.status !== 0 || !ls.stdout.trim()) return res.status(404).json({ error: 'backup not found on Drive' });
    res.setHeader('Content-Disposition', `attachment; filename="${name}"`);
    res.setHeader('Content-Type', 'application/gzip');
    const child = spawn(RCLONE_BIN, [...rcloneConfigArg(), 'cat', `${BACKUP_REMOTE}/${name}`]);
    child.stdout.pipe(res);
    child.on('error', err => { console.error('[restore] rclone cat error:', err.message); res.destroy(); });
    child.on('close', code => { if (code !== 0) { try { res.destroy(); } catch {} } });
  } catch (e) { next(e); }
});

// DELETE /api/backups/:name — remove an archive from Drive (laptop only,
// because rclone write ops require the local rclone binary).
app.delete('/api/backups/:name', async (req, res, next) => {
  try {
    const name = safeBackupName(req.params.name);
    if (!name) return res.status(400).json({ ok: false, error: 'invalid backup name' });
    if (!RCLONE_BIN || IS_VERCEL) {
      return res.status(503).json({ ok: false, error: 'Delete is only available on the operator laptop.' });
    }
    const rm = spawnSync(RCLONE_BIN, [
      ...rcloneConfigArg(), 'delete', `${BACKUP_REMOTE}/${name}`, '--drive-use-trash=false',
    ], { encoding: 'utf8' });
    if (rm.status !== 0) {
      return res.json({ ok: false, error: 'Delete failed: ' + (rm.stderr || '').toString().slice(0, 300) });
    }
    appendAudit({ userId: 'system', userName: '—', action: 'backup.delete', target: name, details: 'Deleted archive from Drive' });
    res.json({ ok: true });
  } catch (e) { next(e); }
});

// POST /api/backups/:name/restore — load an archive into a STAGING schema.
// Never overwrites live `public`.  Refuse on Vercel / when psql missing.
app.post('/api/backups/:name/restore', async (req, res, next) => {
  try {
    const name = safeBackupName(req.params.name);
    if (!name) return res.status(400).json({ ok: false, error: 'invalid backup name' });
    if (!PSQL_BIN || IS_VERCEL) {
      return res.status(503).json({ ok: false, error: 'Restore requires psql + the operator laptop. This server cannot restore.' });
    }
    if (!RCLONE_BIN) return res.status(503).json({ ok: false, error: 'rclone not available on this server.' });

    const workDir = join(process.env.TMPDIR || process.env.TMP || '/tmp', 'mm-restore');
    mkdirSync(workDir, { recursive: true });
    const gzPath  = join(workDir, name);
    const sqlPath = join(workDir, name.replace(/\.gz$/, ''));

    // 1) pull archive from Drive
    const pull = spawnSync(RCLONE_BIN, [...rcloneConfigArg(), 'copyto', `${BACKUP_REMOTE}/${name}`, gzPath], { encoding: 'utf8' });
    if (pull.status !== 0) return res.json({ ok: false, error: 'Failed to fetch archive: ' + (pull.stderr || '').toString().slice(0, 300) });

    // 2) gunzip on disk
    writeFileSync(sqlPath, gunzipSync(readFileSync(gzPath)));

    // 3) recreate staging schema, load
    const env = { ...process.env, PGPASSWORD: (() => { try { return new URL(process.env.DATABASE_URL).password || ''; } catch { return ''; } })() };
    const dsn = process.env.DATABASE_URL || '';
    const drop = spawnSync(PSQL_BIN, [dsn, '-c', `DROP SCHEMA IF EXISTS "${RESTORE_SCHEMA}" CASCADE; CREATE SCHEMA "${RESTORE_SCHEMA}";`], { encoding: 'utf8', env });
    if (drop.status !== 0) return res.json({ ok: false, error: 'Staging schema reset failed: ' + (drop.stderr || '').toString().slice(0, 300) });
    // Re-point the loaded tables into the staging schema. The dump uses
    // unqualified `"table"` identifiers (which would resolve to live `public`),
    // so we rewrite them to `"public_restore"."table"` BEFORE executing — this
    // guarantees the live schema is never touched.
    const scrubbed = readFileSync(sqlPath, 'utf8')
      .replace(/DROP TABLE IF EXISTS "([^"]+)"/g, `DROP TABLE IF EXISTS "${RESTORE_SCHEMA}"."$1"`)
      .replace(/CREATE TABLE "([^"]+)"/g, `CREATE TABLE "${RESTORE_SCHEMA}"."$1"`)
      .replace(/INSERT INTO "([^"]+)"/g, `INSERT INTO "${RESTORE_SCHEMA}"."$1"`);
    writeFileSync(sqlPath + '.staged.sql', scrubbed);
    const load2 = spawnSync(PSQL_BIN, [dsn, '-v', `ON_ERROR_STOP=0`, '-f', sqlPath + '.staged.sql'], { encoding: 'utf8', env });
    if (load2.status !== 0) {
      return res.json({ ok: false, error: 'Load into staging failed: ' + (load2.stderr || '').toString().slice(0, 300) });
    }
    appendAudit({ userId: req.mm?.id || 'system', userName: req.mm?.name || '—', action: 'backup.restore', target: name, details: `Loaded into staging schema "${RESTORE_SCHEMA}" (verify before promoting).` });
    // 4) report table row counts in staging so the admin can eyeball
    const cnt = spawnSync(PSQL_BIN, [dsn, '-t', '-c', `SELECT string_agg(format('%s:%s', table_name, (xpath('/rowcount/text()', query_to_xml(format('SELECT count(*) AS rowcount FROM %I.%I', '${RESTORE_SCHEMA}', table_name), false, true, '')))[1]::text), ', ') FROM information_schema.tables WHERE table_schema='${RESTORE_SCHEMA}';`], { encoding: 'utf8', env });
    res.json({
      ok: true,
      stagingSchema: RESTORE_SCHEMA,
      note: 'Archive loaded into staging schema. Inspect it, then promote from the operator laptop.',
      counts: cnt.stdout?.trim() || '',
    });
  } catch (e) { next(e); }
});
